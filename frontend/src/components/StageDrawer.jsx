...
const COUNTRY_NAMES = {
  NG: 'Nigeria', US: 'United States', GB: 'United Kingdom', PH: 'Philippines',
  GH: 'Ghana', KE: 'Kenya', ZA: 'South Africa', CA: 'Canada', AU: 'Australia',
  IN: 'India', NZ: 'New Zealand', JM: 'Jamaica', TT: 'Trinidad & Tobago',
  BB: 'Barbados', ZW: 'Zimbabwe', UG: 'Uganda', TZ: 'Tanzania', RW: 'Rwanda',
  CM: 'Cameroon', CI: "Côte d'Ivoire", SN: 'Senegal', ET: 'Ethiopia',
  // add more as needed
}
...
export default function StageDrawer({ stage, leads, onClose, onLeadDeleted }) {
  ...
  const [tab, setTab] = useState('leads') // 'leads' | 'countries'
  ...
  const stageLeads = (leads || []).filter(l =>
    !deleted.includes(l.contactId) &&
    (isAll || (l.currentStage || '').toLowerCase() === (stage || '').toLowerCase())
  )
        ...
        <div style={{ padding: '20px 24px 0', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '14px' }}>
            <div>
              ...
              <div style={{ fontSize: '16px', fontWeight: 400, color: '#fff', letterSpacing: '-0.01em' }}>{isAll ? 'All Applicants' : stage}</div>
          ...
          <div style={{ display: 'flex', gap: '2px' }}>
            ...
            <button style={tabStyle('countries')} onClick={() => setTab('countries')}>By Country</button>
          </div>
        </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '3px', flexWrap: 'wrap' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                          ...
                          <span style={{ fontSize: '10px', color: '#666' }}>{lead.source || 'Unknown'}</span>
                        </div>
                        {lead.country && (
                          <span style={{ fontSize: '10px', color: '#666', background: 'rgba(255,255,255,0.05)', padding: '1px 6px', borderRadius: '3px' }}>
                  ...
                  <div style={{ marginTop: '8px', display: 'flex', justifyContent: 'flex-end' }}>
                    {confirmId === lead.contactId ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <button onClick={() => handleDelete(lead.contactId)} disabled={deleting === lead.contactId} style={{ fontSize: '10px', background: 'rgba(220,50,50,0.15)', border: '1px solid rgba(220,50,50,0.3)', borderRadius: '4px', color: '#e05555', padding: '3px 10px', cursor: 'pointer', fontFamily: "'Inter', system-ui, sans-serif", fontWeight: 600 }}>
                          {deleting === lead.contactId ? 'Deleting…' : 'Yes, delete'}
                        </button>
                        <button onClick={() => setConfirmId(null)} style={{ fontSize: '10px', background: 'transparent', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '4px', color: '#555', padding: '3px 10px', cursor: 'pointer', fontFamily: "'Inter', system-ui, sans-serif" }}>Cancel</button>
                      </div>
                    ) : (
                      <button onClick={() => setConfirmId(lead.contactId)} style={{ fontSize: '10px', background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '4px', color: '#444', padding: '3px 10px', cursor: 'pointer', fontFamily: "'Inter', system-ui, sans-serif", letterSpacing: '0.04em', transition: 'all .15s' }}
          ...
          {tab === 'countries' && (
            <div style={{ padding: '16px 24px' }}>
              {countryList.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px', color: '#444', fontSize: '13px' }}>No country data yet</div>
              ) : (
                <>
                  <div style={{ fontSize: '10px', fontWeight: 600, color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '14px' }}>
                    {countryList.length} countries represented
                  </div>
                  {countryList.map(([country, count]) => {
                    const pct = Math.round((count / stageLeads.length) * 100)
                    const barWidth = Math.round((count / maxCount) * 100)
                    return (
                      <div key={country} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                        <div style={{ fontSize: '12px', color: '#b0b0b0', width: '140px', flexShrink: 0, fontWeight: 400 }}>{country}</div>
                        <div style={{ flex: 1, height: '3px', background: '#1a1a1a', borderRadius: '2px', overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${barWidth}%`, background: '#d4af37', borderRadius: '2px', transition: 'width .4s ease' }} />
                        </div>
                        <div style={{ fontSize: '13px', fontWeight: 500, color: '#fff', width: '24px', textAlign: 'right' }}>{count}</div>
                        <div style={{ fontSize: '10px', color: '#555', width: '32px', textAlign: 'right' }}>{pct}%</div>
                      </div>
                    )
                  })}
                </>
